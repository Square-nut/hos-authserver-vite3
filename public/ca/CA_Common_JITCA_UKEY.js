...
function ca_key_IE() {
    ...
    try {
        ...
        JITDSignOcx.setAttribute("style","position:absolute;left:0px;top:-1000px;width:1px;height:1px;");
    ...
    var InitParam = "<?xml version=\"1.0\" encoding=\"utf-8\"?>"+"<authinfo><liblist>"+
        "<lib type=\"SKF\" version=\"1.1\" dllname=\"U3hjYUNzcDExXzMwMDBHTS5kbGw=\" ><algid val=\"SHA1\" sm2_hashalg=\"sm3\"/></lib>"+
    ...
    function SignedData(content,key){  
        if (tempSc == "")
            tempSc = key;
        if (selectCert(key)) {     
            var result = JITDSignOcx.AttachSignStr("",content);
            ret = JITDSignOcx.GetErrorCode();
            if (ret != 0) {
                message = JITDSignOcx.GetErrorMessage(ret);
    ...
    function GetUniqueID(cert,key) {
        if (selectCert(key)) {
            var result = JITDSignOcx.GetCertInfo("SC",7,"1.2.156.10260.4.1.1");
            ret = JITDSignOcx.GetErrorCode();
            if (ret != 0) {
                message = JITDSignOcx.GetErrorMessage(ret);
                alert("签名失败，错误码：" + ret + " 错误信息：" + message);
    ...
    function GetPicBase64Data(key) {
        if (key == "CN=测试4, OU=140428195201032536, L=太原, S=山西省, C=CN")
        {
        ...
        if (key == "CN=测试2, OU=140428199005203247, L=太原, S=山西省, C=CN")
        {
...
function ca_key_CHROME() {
    ...
    function GetUserList(){
        var InitParam = "<?xml version=\\\"1.0\\\" encoding=\\\"utf-8\\\"?><authinfo><liblist>";
        InitParam +="<lib type=\\\"SKF\\\" version=\\\"1.1\\\" dllname=\\\"U3hjYUNzcDExXzMwMDBHTS5kbGw=\\\" ><algid val=\\\"SHA1\\\" sm2_hashalg=\\\"sm3\\\"/></lib>";
        InitParam +="<lib type=\\\"SKF\\\" version=\\\"1.1\\\" dllname=\\\"bXRva2VuX2dtMzAwMF9wY3NjLmRsbA==\\\" ><algid val=\\\"SHA1\\\" sm2_hashalg=\\\"sm3\\\"/></lib>";
        InitParam +="</liblist></authinfo>";
    ...
    function SignedData(content,key) {
        if (tempSc == "")
            tempSc = key;
        if (selectCert(key)){
            var result = JITHTTPCLIENT.AttachSignStr("",content);
            ret = JITHTTPCLIENT.GetErrorCode();
            if (ret != 0) {
                message = JITHTTPCLIENT.GetErrorMessage(ret);
                alert("签名失败，错误码：" + ret + " 错误信息：" + message);
    ...
    function GetUniqueID(cert,key) {
        if (selectCert(key)){
            var result = JITHTTPCLIENT.GetCertInfo("SC",7,"1.2.156.10260.4.1.1");
            ret = JITHTTPCLIENT.GetErrorCode();
            if (ret != 0) {
                message = JITHTTPCLIENT.GetErrorMessage(ret);
                alert("签名失败，错误码：" + ret + " 错误信息：" + message);
