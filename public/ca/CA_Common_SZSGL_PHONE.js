///包装圣格灵的key值为userCertCode-certSN-userCode
var ca_key = (function() {
    
    var Const_Vender_Code = "SZSGL";
  		...
  		$.ajax({
    		...
    		success: function(ret) {
            	if (ret && ret.retCode == "0") {
                	result = ret;
            	} else if ((ret.retCode == "-2")||(ret.retCode == "-3")) {
	            	//不处理	    
	            } else {
	            	alert(ret.retMsg);
            	}
    ...
    function iGetCert(key) {
        var userCode = key.split("-")[2];
	    return iGetData("GetCert",userCode);
    }
    ...
    function iGetSeal(key) {
        var userCode = key.split("-")[2];
	    return iGetData("GetSeal",userCode);
    }
	...
	function iAuthSignS(inData,key,token) {
		var result = iGetData("AuthSignS", inData, key, token);
		return result;
	}
	...
	function oSignedData(inData, key) {
		key = iGetRealKey(key);
		var token = "";
		return oAuthSign(inData,key,token);
	}
	
	function oSignedOrdData(inData, key) {
		key = iGetRealKey(key);
		var token = "";
		return oAuthSign(inData,key,token);
	}
	...
	function oSignedDataS(inData, key) {
		key = iGetRealKey(key);
		var token = "";
		return oAuthSignS(inData,key,token);
	}
	...
	function oAuthSignS(inData,key,token) {
		var ret = "";
		var rtn = iCheckAuthSignToken(key, token);
		if (rtn.retCode == "0") {
			var token = rtn.signToken || "";
			if(token != "") {
				var rtn = iAuthSignS(inData,key,token);
				if ((rtn.retCode == "0")&&(rtn.signStatus = "FINISH")) {
					ret = rtn.signResult;
				}else {
					Global_Last_Login_CertInfo = "";
    ...
    // 用户登录
	function oLogin(strFormName, strCertID, strPin) {
        return false;
	}
	...
	function oGetUsrSignatureInfo(key) {
	    ...
	    var ret = iGetData("GetCertByUserCode",key,"");
	...
	function oCheckPWD(strCertID, strPin) {
	    ...
	    // 校验密码
	    var ret = oLogin(strCertID, strPin);
	    if (!ret) {
	        return "登录失败!";
	    }
	    return "";
	...
	function oLoginForm(paraObj,callback,arr1,arr2) {
		...
		if (judgeIsIE())
		{
			...
			var rtnStr = window.showModalDialog("../csp/dhc.certauth.login.qrcode.csp",para,
    ...
    return {
        ...
        SignedDataS: function(contentHash, key) {
            return oSignedDataS(contentHash, key)
        },
