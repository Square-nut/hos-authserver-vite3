...
var ca_key = (function() {
    ...
    /***********  5.1.  常量 2012.04.08      ***********/
    /***************************************************/
    var PKCSTimeStampURL="";    //http://192.168.4.169/NETCATimeStampServer/TSAServer.jsp
	...
	var data = ajaxDATA('String', 'CA.ConfigService', 'GetServiceCfgByCode', "NETCA", "UKEY");
	jQuery.ajax({
		type: 'POST',
		dataType: 'text',
		...
		success: function (ret) {
			if (ret != ""){
	            ...
	            PKCSTimeStampURL = arr["TSLocation"];
	            if (PKCSTimeStampURL == "")
	            {
		            $.messager.alert("提示","请配置签名服务中的时间戳服务地址");
		        }
			}else {
    ...
    /*CA证书的CN*/
    var CASTR = new Array("NETCA", "CN=GDCA", "CN=WGCA");
    ...
    /***************************************************/
    try {
        SecuInter = document.createElement("object");
        SecuInter.setAttribute("width",1);
        SecuInter.setAttribute("height",1);
        SecuInter.setAttribute("id","oSecuInter");
        SecuInter.setAttribute("name","oSecuInter");
        SecuInter.setAttribute("style","position:absolute;left:0px;top:-1000px;width:1px;height:1px;");
        if (window.ActiveXObject || "ActiveXObject" in window) {
            SecuInter.setAttribute("classid","clsid:11E27019-33D7-4B76-B73C-11DB7F496813");
        } else {
            SecuInter.setAttribute("type","application/x-xtx-axhost");
            SecuInter.setAttribute("clsid","{11E27019-33D7-4B76-B73C-11DB7F496813}");
        }
        document.documentElement.appendChild(SecuInter);
    } catch (e) {
    ...
    */
    ...
    /***    5.4.12	PKCS#7时间戳签名 2021-01-27 ***/
    // 参数   bContent 签名内容 ，字符串
    //         oCert Secuinter组件的证书对象 
    //         tsaUrl   时间戳地址 字符串
    //         IsNotHasSource 是否detach原文，false表示带原文，true表示不带原文
    //         pwd 证书密码，字符串，可选
    //        (bContent, oCert, IsNotHasSource, pwd)
    function signPKCS7WithTSAEx(bContent, oCert, IsNotHasSource,pwd) {
        if (bContent == "") {
            alert("原文内容为空!");
            return null;
        }
        ...
        var signature;
        //获取证书便秘吗
        var signCertEncode= getX509CertificateInfo(oCert, 0);
        if (signCertEncode == null || signCertEncode == "") {
            alert("获取签名证书编码失败!");
            return null;
        }
        ...
        try {
        //获取NETCA 中间件的 证书对象
        //oCert和certobj 是不同组件的对象 需要转换
            certobj.Decode(signCertEncode);
        } catch (e) {
            alert("解码证书失败!");
            return null;
        }
        ...
        if (sign1obj.SetSignCertificate(certobj, pwd, false) == false)
        {
            alert("设置签名证书失败!");
            return null;
        }
        ...
        sign1obj.Detached = IsNotHasSource;

        signature = sign1obj.SignWithTSATimeStamp(tsaUrl,tbs,NETCAPKI_CMS_ENCODE_BASE64);

        return signature;
    }

    /***    5.4.12	PKCS#7时间戳签名 2010-09-02 ***/
    function signPKCS7WithTSA(bContent, oCert, IsNotHasSource, pwd) {
        if (bContent == "") {
            alert("原文内容为空!");
            return null;
        }
        ...
        //设置信任证书
        //证书编码不同项目需要变更
        var TrustArchorCerts = new ActiveXObject("SecuInter.X509Certificates");
        if (TrustArchorCerts == null) {
            alert("设置信任时间戳证书失败");
            return null;
        }
        var  tsaCert="";
        var tsaCertObj = getX509CertificateByString(tsaCert) 
        if (tsaCertObj == null) {
            alert("设置信任时间戳证书失败");
            return null;
        }
        TrustArchorCerts.Add(tsaCertObj);
        initialObj.SetTrustArchor(TrustArchorCerts);
        ...
        //UTF8编码
        oSignedData.content = initialObj.UTF8Encode(bContent);
        oSignedData.Detached = IsNotHasSource;
    ...
    */
    ...
    function getSignCertEx(key){
        return getX509CertificateByValue(SECUINTER_CURRENT_USER_STORE,SECUINTER_MY_STORE,SECUINTER_CERTTYPE_SIGN,SECUINTER_NETCA_ALL,9,key);
    }
    ...
    function SignedData(content, key) {
        if ("" == content)
            return "";
        var oCert=getSignCertEx(key);
        if(oCert==null){
            alert("签名错误，未找到证书!");
            return "";
        }
        try{
            //return signPKCS7ByCert(content,oCert,false,"");//带源文pkcs7签名,此处密码去掉了，提高300毫秒
            return signPKCS7WithTSAEx(content,oCert,false,"");//带源文带时间戳的pkcs7签名
        }
        catch (e){
    ...
    function GetUniqueID(cert,key) // 如果cert是数字证书的Base64编码,则我们可以实现
    {
    ...
    function getUsrSignatureInfo(key) {
        ...
        usrSignatureInfo["certificateNo"] =  getX509CertificateInfo(oCert,2);// 证书号;
        ...
        usrSignatureInfo["UsrCertCode"] =  getX509CertificateInfo(oCert,9);// 证书唯一编码,由于CA公司不同,所以此处为可区别证书的编码(可为uKeyNo或CertificateNo等等)
    ...
    function Login(form, key, pwd) {
        pwd = pwd || "";
        if (iKeyExist(key)&&(SOF_IsLogin(key))&&(pwd == "")) {
            return true;
        }
        if(pwd==""){
            alert("请输入密码");
        ...
        try{
            return signPKCS7WithTSAEx("test",oCert,false,pwd);
        }
        catch (e){
            alert("密码错误，请检查是否插入KEY，或者KEY的密码是否正确");
            return "";
        }
        return true;
    }
	...
	function CheckPWD(strCertID, strPin) {
        ...
        var oCert = getSignCertEx(strCertID);
        if (oCert == null) {
            return "未找到证书";
        }
        
        try{
            return signPKCS7WithTSAEx("test",oCert,false,strPin);
        }
        catch (e){
            return "密码错误，请检查是否插入KEY，或者KEY的密码是否正确";
        }
        return "";
	}
