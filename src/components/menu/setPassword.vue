<template>
  <div @click.stop="flagHide()" class="change-pass" :class="personUuid ? 'setWidth' : ''">
...
export default {
            ...
            validator: (rule, value, callback) => {
              if (value && value !== "") {
                if (value.length < this.pwdPolicy.minSize) {
                  callback(
                    new Error(this.$t("新密码长度不符合密码策略要求"))
                  );
                } else {
                  if(this.pwdPolicy.complexity && this.pwdPolicy.complexity.indexOf("number")!=-1){
                    var val =/\d+/;
                    if(!val.test(value)){
                      callback(
                  ...
                  if(this.pwdPolicy.complexity && this.pwdPolicy.complexity.indexOf("letter")!=-1){
                    var val =/[a-zA-Z]+/;
                    if(!val.test(value)){
                      callback(
                          new Error(this.$t("新密码不符合密码复杂度要求"))
                  ...
                  if(this.pwdPolicy.complexity && this.pwdPolicy.complexity.indexOf("lowercaseLetter")!=-1){
                    var val =/[a-z]+/;
                    if(!val.test(value)){
                      callback(
                          new Error(this.$t("新密码不符合密码复杂度要求"))
                  ...
                  if(this.pwdPolicy.complexity && this.pwdPolicy.complexity.indexOf("capitalLetter")!=-1){
                    var val =/[A-Z]+/;
                    if(!val.test(value)){
                      callback(
                          new Error(this.$t("新密码不符合密码复杂度要求"))
                  ...
                  if(this.pwdPolicy.complexity && this.pwdPolicy.complexity.indexOf("pecialCharacters")!=-1){
  ...
  created() {
			if(document.querySelector('.login-loading-mask')) document.querySelector('.login-loading-mask').style.display = 'none'
  ...
  methods: {
    ...
    // 沿用密码
    useLastPwd() {
      this.$api("sys-password.useLastPwd",{personUuid:this.personUuid}).then((res) => {
        if (res && res.code == 200) {
