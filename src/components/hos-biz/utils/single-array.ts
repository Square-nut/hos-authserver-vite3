 ...
 * @Last Modified time: 2022-03-12 11:37:32
 */
 ...
 * 如果存在key就按照object处理
 */
export default class SingleArray<T = Record<string, unknown>> {
	key?: string;
	arr: T[];
	...
	constructor(arr: T[], key?: string) {
		...
		if (key) {
			...
			this.arr = stringArr.map((v) =>
				arr.filter((c) => (c as Record<string, unknown>)[key] === v)[0],
			) as T[];
		} else {
			this.arr = arr ? ([...new Set(arr)] as T[]) : [];
		}
	}
	has(item: T | T[], isEvery?: boolean): boolean {
		if (Array.isArray(item)) {
			...
			return this.key
				? this.arr.some(
						(v) =>
							(v as Record<string, unknown>)[this.key!] ===
							(item as Record<string, unknown>)[this.key!],
					)
