...
var ca_key = (function() {
	...
	try {
        if (window.ActiveXObject || "ActiveXObject" in window) {
            ...
            picobj = document.createElement("embed");
        }
	...
	function SOF_SignData(CertID, inData) {
	    var result = XTXAPP.SOF_SignData(CertID, inData);
        if (result == "") {
			var errorInfo = SOF_GetLastErrorMsg();
			alert("调用BJCA接口获取签名结果失败："+errorInfo);
		}
		return result;
	}
	...
	function GetSignCert(key) {
	    var result = SOF_ExportUserCert(key, KEY_SIGNATURE);
        if (result == "") {
			var errorInfo = SOF_GetLastErrorMsg();
			alert("调用BJCA接口获取签名证书失败："+errorInfo);
		}
		return result;
	}
	...
	function GetPicBase64Data(key) {
	    return picobj.GetPic(key);
	}
    
	function HashData(InData) {
        //兼容新老驱动
        try {
            var hashdata = picobj.Hash(InData);
        } catch (e) {
            var hashdata = "";
        }
        if (hashdata == "") hashdata = XTXAPP.SOF_HASHDATA(1,InData);
        return hashdata;
	}
	...
	function GetUniqueID(cert) {
        ...
        if (uniqueID == "") {
			var errorInfo = SOF_GetLastErrorMsg();
			alert("调用BJCA接口获取用户唯一标识失败："+errorInfo);
		}
        
        //1@5009SF0140109199512210517     1@215009SF0110227198907016119
        if (/^\d{1,2}@\d{1,}SF0\d{1,}$/.test(uniqueID)) {
			return "SF" + uniqueID.split("SF0")[1];
		} else {
			return uniqueID;
		}
	}
