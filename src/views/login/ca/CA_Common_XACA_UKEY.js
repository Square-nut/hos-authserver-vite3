...
var ca_key = (function () {
    ...
    function Login(form_,key,password_) {
        ...
        SecCtrl.KS_VerifyUserPIN(password_);
        if(SecCtrl.KS_GetLastErrorCode() != 0){
            alert(SecCtrl.KS_GetLastErrorMsg());
            return false;
        }
    ...
    function GetUniqueID(cert) {
        var result = ""
        var _key = CA_GetCertInfo(cert,2);
        if (CA_setDefaultKey(_key)) {
            return CA_GetUniqueID(cert);
        } else {
            alert("设置默认签名证书失败！")
            return result;
        }
    ...
    function getUsrSignatureInfo(key){
        ...
        usrSignatureInfo["UsrCertCode"] = CA_GetUniqueID(certB64);
