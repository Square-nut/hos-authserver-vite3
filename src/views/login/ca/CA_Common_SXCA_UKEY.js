var ca_key = (function() {
    ...
    HebcaClient.prototype = {
        ...
        ***********************************************************/
        ...
        GetDigest: function(text) {
            ...
            return (this._GetClientCtrl()).Util.HashText(text,2);
        },
        //调取key进行数字签名
        Sign: function(keySN,signalg) {
            ...
            var b64Cert = d.SelectSignCert().GetCertB64();
            var pkcs7 = this._GetClientCtrl().CreatePkcs7();
            pkcs7.AddRecipientCert(b64Cert);
            return pkcs7.SignText(0, signalg, 2);
            //return c.SignText(signalg, 1);
        },
        //得到签名证书内容B64格式
        ...
        GetG: function(cert) {
            ...
            for (var i=0;i<dCnt;i++) {
                ...
                if (cert==tmpDev.SelectSignCert().GetCertB64()) {
                    return tmpDev.SelectSignCert().GetCertExtensionByOid("1.2.86.11.7.11");
                }
            }
        },
        //得到证书唯一项
    ...
    ********************************************************************************/
    ...
    try {
        ...
        {
            //document.writeln('<OBJECT classid="CLSID:F34B7272-DA4D-4DCB-B3FE-164363F020F0" id=TZUtil name=TZUtil style="HEIGHT: 0px; LEFT: 0px; TOP: 0px; WIDTH: 0px; display: none" width=0 height=0 VIEWASTEXT></OBJECT>');
            TZUtil = document.createElement("object");
            TZUtil.setAttribute("width",1);
            TZUtil.setAttribute("height",1);
            TZUtil.setAttribute("id","TZUtil");
            TZUtil.setAttribute("style","position:absolute;left:0px;top:-1000px;width:1px;height:1px;");
            TZUtil.setAttribute("classid","clsid:F34B7272-DA4D-4DCB-B3FE-164363F020F0");
            document.documentElement.appendChild(TZUtil);
        }
        else
        {
            alert('请注意，非IE浏览器可能无法加载控件！');
        }
        var TZUtil = document.getElementById("TZUtil");
    } catch(e) {
        alert(e.message);
    }
    ...
    {
        return hebcaClient.Sign(key,content);
    ...
    function GetIdentityID(key)
    {
        ...
        /*try {
            ...
            //return idNo.substr(idNo.length-21);
    ...
    {
        var vernderId  = -3;
        var SealBase64 = TZUtil.ReadESeal(vernderId);
        return SealBase64;
    }
    ...
    /*function sealAction(content, key) {
        if (content == "" || typeof(content) == "undefined" || content == null) {
            ...
            try {
                ...
                //sealClient.Login(sealSN, pwd)
                //最后一个参数设为true则前端生成时间戳，设为false在服务断直接生成
                //signresult = sealCtrl.SignAndSealWithoutTimeStampCert(content,sealSN,1,false,true);
                //timestamp = sealCtrl.GetTimeStamp();
                //tscert= sealCtrl.GetTimeStampcert();
                //return signresult+";;;timestamp="+timestamp+"||tscert="+tscert;
            } catch (e) {
                alert(e.message);
            }
        }
    ...
    function getUsrSignatureInfo(key)
    {
        ...
        var count = hebcaClient._GetClientCtrl().GetDeviceCount();
        if (count > 1) {
