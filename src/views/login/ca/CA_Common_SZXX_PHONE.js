var ca_key = (function() {
	...
	// todo: 授权自动签名
	function iSignData(inData,key,token, episodeID, attachJson) {
		token = token||"";
	...
	function oSignedData(content, key, episodeID, attachJson) {
		key = iGetRealKey(key);
	    return iSignData(content, key, "", episodeID, attachJson);
	}
	
	function oSignedOrdData(content, key, episodeID, attachJson) {
		key = iGetRealKey(key);
	    return iSignData(content, key, "", episodeID, attachJson);
	}
	...
	function oLoginForm(paraObj,callback,arr1,arr2) {
		...
		else
		{
			...
			createModalDialog("loginQrcode","扫码页面",1000,700,'loginQrcodeFrame',content,callback,arr1,arr2);		
		}
	}
    ...
    return {
        ...
        SignedData: function(contentHash, key, episodeID, attachJson) {
            return oSignedData(contentHash, key, episodeID, attachJson)
        },
        SignedOrdData: function(contentHash, key, episodeID, attachJson) {
            return oSignedOrdData(contentHash, key, episodeID, attachJson)
        },
